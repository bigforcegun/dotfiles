---
name: maintenance-omo
description: Maintenance runbook for Oh My OpenAgent (OMO) in this dotfiles workspace. Use whenever the user wants to maintain, assess, update, validate, or repair OMO, its OpenCode integration, its configured MCP surface, or its configuration links, even without naming this skill. Do not use for generic OpenCode usage questions, standalone MCP work, or unrelated dotfiles edits.
---
...
Use `scripts/maintenance-omo` for deterministic checks:

- `scripts/maintenance-omo status`
- `scripts/maintenance-omo update-check`
- `scripts/maintenance-omo config-audit`
- `scripts/maintenance-omo verify [--smoke]`
