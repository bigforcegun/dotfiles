...
1. Open [[{{HUB}}]] — the hub. There is no generated index; the corpus is whatever
   links back to it.
...
| Target | Form |
|---|---|
| The hub | `[[{{HUB}}]]` — every entry note carries it, right under the H1 |
...
- An entry note without `[[{{HUB}}]]` is an error, not a warning — it drops out of the
