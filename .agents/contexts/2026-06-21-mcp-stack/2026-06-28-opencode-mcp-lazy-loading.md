...
Связано с [[2026-06-28-mcp-stack-plan]]: закрывает дыру «толщина контекста = по наименее способному агенту».
...
Смежный плагин в shortlist: [`mailshieldai/opencode-canvas`](https://github.com/mailshieldai/opencode-canvas)
/ `@mailshieldai/opencode-canvas` — интерактивные terminal canvas/TUI для OpenCode
(`canvas_spawn`, `canvas_update`, `canvas_selection`, `canvas_close`) через tmux split panes.
Это **не** решение MCP lazy-loading, но полезный кандидат для отдельного UI/agent-workflow трека;
проверить установку из npm, требования `bun` + `tmux`, permission `canvas_*`, совместимость с OMO и
стоимость дополнительного tool-surface.
...
- **смежное** — оптимизации контекста, agent-конфиги, совместимость с `oh-my-openagent`;
  отдельно проверить canvas/TUI-плагины вроде `mailshieldai/opencode-canvas`.
